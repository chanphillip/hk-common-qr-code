...
(function() {
	...
	// CRC16-CCIT
	// ref: https://gist.github.com/chitchcock/5112270
	...
	var HKCommonQRCode = function(payload) {
		...
		this.toString = function() {
			...
			switch (_payload.init) {
				...
				default:
					// TODO: check validity
					output += lz(_payload.init, 2);
			}

			// merchant account information
			output += lz(_payload.merchant.operator, 2);

			var merchantOutput = Object.keys(_payload.merchant.info).map(function(key) {
				switch (key) {
					case 'guid':
						return '00' + lz(_payload.merchant.info[key].length, 2) + _payload.merchant.info[key];
					default:
						return lz(key, 2) + lz(_payload.merchant.info[key].length, 2) + _payload.merchant.info[key];
				}
			}).join('');
			...
			output += Object.keys(_payload.additional).map(function(key) {
				switch (key) {
					case 'category':
						return '5204' + lz(_payload.additional[key], 4);
					case 'country':
						return '5802' + lz(_payload.additional[key], 2);
					case 'name':
						return '59' + lz(_payload.additional[key].length, 2) + _payload.additional[key];
					case 'city':
						return '60' + lz(_payload.additional[key].length, 2) + _payload.additional[key];
					case 'postal':
						return '61' + lz(_payload.additional[key].length, 2) + _payload.additional[key];
					case 'languageTemplate':
						var value = _payload.additional[key].map(function(languagePayload) {
					...
					case 'currency':
						return '5303' + parseCurrency(_payload.additional[key]);
					case 'amount':
						var value = _payload.additional[key].toString();
						return '54' + lz(value.length, 2) + value;
					case 'tip':
						return '5502' + lz(_payload.additional[key], 2);
					case 'feeFixed':
						return '56' + lz(_payload.additional[key].length, 2) + _payload.additional[key];
					case 'feePercentage':
						return '57' + lz(_payload.additional[key].length, 2) + _payload.additional[key];
					case 'data':
						var dataPayload = _payload.additional[key]
			...
			// calculate CRC
			output += '6304';
			output += lz(crc16(output).toString(16), 4);			// including 6304
